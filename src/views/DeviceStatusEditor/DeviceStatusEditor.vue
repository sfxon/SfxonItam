...
async function loadDeviceStatus(id: number): Promise<void> {
    try {
        ...
        const data = await fetchDeviceStatus(id)
...
async function save() {
    ...
    if(submitSuccess !== false && typeof deviceStatusId.value === 'undefined') {
        window.location.href = generateUrl('apps/sfxonitam/device-status/detail?deviceStatusId=' + submitSuccess)
    }
}
                    ...
                    <div :class="[SfxonEditorStyles.sfxonFormSection, $style.formSection1]">
                        <div :class="[SfxonEditorStyles.sfxonFormColumn]">
                            <SfxonEditorFormInput
                                ...
                                type="text"
                                :placeholder="t('sfxonitam', 'e.g. In use')"
