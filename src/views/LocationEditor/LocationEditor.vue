...
async function loadLocation(id: number): Promise<void> {
    ...
    try {
        ...
        generalError.value = t('sfxonitam', 'Location could not be loaded.')
        console.error('Error while loading location:', e)
    } finally {
...
async function save() {
    ...
    if(submitSuccess !== false && typeof locationId.value === 'undefined') {
        window.location.href = generateUrl('apps/sfxonitam/location/detail?locationId=' + submitSuccess)
    }
}
                    ...
                    <div :class="[SfxonEditorStyles.sfxonFormSection, $style.formSection1]">
                        <div :class="[SfxonEditorStyles.sfxonFormColumn]">
                            <SfxonEditorFormInput
                                ...
                                type="text"
                                :placeholder="t('sfxonitam', 'e.g. City of Mindelheim')"
