...
async function loadMerchant(id: number): Promise<void> {
    ...
    try {
        const d = await fetchMerchant(id)
        name.value = d.name ?? ''
        comment.value = d.comment ? d.comment : ''
    } catch (e: any) {
        generalError.value = t('sfxonitam', 'Händler konnte nicht geladen werden.')
        console.error('Fehler beim Laden des Händlers:', e)
    } finally {
    ...
    <NcContent app-name="sfxonitammerchanteditor">
        ...
        <NcAppContent :class="$style.content">
            <div :class="$style.form">
                <h2>
                    {{ isEditMode
                        ? t('sfxonitam', 'Händler bearbeiten')
                        : t('sfxonitam', 'Händler erfassen') }}
                </h2>
