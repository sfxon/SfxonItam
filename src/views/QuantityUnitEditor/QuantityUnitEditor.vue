...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    {
    label: t('sfxonitam', 'Quantity Units'),
...
async function loadQuantityUnit(id: number): Promise<void> {
    ...
    try {
        const data = await fetchQuantityUnit(id)
...
async function save() {
    ...
    if(submitSuccess !== false && typeof quantityUnitId.value === 'undefined') {
        window.location.href = generateUrl('apps/sfxonitam/quantity-unit/detail?quantityUnitId=' + submitSuccess)
    }
}
...
<template>
    <NcContent app-name="sfxonitamquantityuniteditor">
        <NcAppNavigation>
            <NcAppNavigationList>
                <NcAppNavigationNew
                :text="t('sfxonitam', 'Add Quantity Unit')"
                @click="addItem"
                >
                    <template #icon>
                    ...
                    <div :class="[SfxonEditorStyles.sfxonFormSection, $style.formSection1]">
                        <div :class="[SfxonEditorStyles.sfxonFormColumn]">
                            <SfxonEditorFormInput
                                ...
                                type="text"
                                :placeholder="t('sfxonitam', 'e.g. Liter')"
